...
void UI_Delete(uint8_t Del_Operate, uint8_t Del_Layer)
{
    ...
    framehead.CRC8         = Get_CRC8_Check_Sum((uint8_t *)&framehead, 4);
	 ...
	 *(uint16_t *)(UI_BUFF + 7 + 6 + 2) = Get_CRC16_Check_Sum(UI_BUFF, 7 + 6 + 2);
...
int Graph_ReFresh(int cnt,...)
{
   ...
   *(uint16_t *)(UI_BUFF + 7 + 6 + cnt * 15) = Get_CRC16_Check_Sum(UI_BUFF, 13 + cnt * 15);
...
int Char_ReFresh(String_Data string_Data)
{
    ...
    *(uint16_t *)(UI_BUFF + 7 + 6 + 45) = Get_CRC16_Check_Sum(UI_BUFF, 7 + 6 + 45);
