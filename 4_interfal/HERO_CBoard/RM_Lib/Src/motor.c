...
float QuickCentering_DEG(float Ang, uint16_t Exp, int8_t round) {
    float Exp_ = (Exp + 4095) % 8192;
    if (Exp_ < Exp)
        return Ang < Exp_ * 0.0439453125f + round * 360.0f ? Exp * 0.0439453125f - 180.0f + round * 360.0f : Exp * 0.0439453125f + round * 360.0f;
    else
        return Ang > Exp_ * 0.0439453125f + round * 360.0f ? Exp * 0.0439453125f + 180.0f + round * 360.0f : Exp * 0.0439453125f + round * 360.0f;
}
