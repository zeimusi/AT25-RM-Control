/**
  ****************************(C) COPYRIGHT 2024 Polarbear*************************
  * @file       usb_task.c/h
  * @brief      通过USB串口与上位机通信
  * @note
  * @history
  *  Version    Date            Author          Modification
  *  V1.0.0     Jun-24-2024     Penguin         1. done

  @verbatim
  =================================================================================

  =================================================================================
  @endverbatim
  ****************************(C) COPYRIGHT 2024 Polarbear*************************
*/
#include "usb_task.h"
#include "usb_device.h"
#include "usbd_cdc_if.h"
#include "usbd_conf.h"

#define USB_OFFLINE_THRESHOLD 1000  // ms USB离线时间阈值
#define USB_CONNECT_CNT 10

// 发送时间间隔，每次发送经过以下时间后发送消息

#define SEND_DURATION_AllRobotHp    10  // ms
#define SEND_DURATION_GameStatus    10  // ms 

#define SEND_DURATION_GameBegin     10  // ms 
#define SEND_DURATION_Vulnerable	10	// ms
#define SEND_DURATION_Radar_Mark    10  // ms

#define USB_RX_DATA_SIZE 256  // byte
#define USB_RECEIVE_LEN 150   // byte
#define HEADER_SIZE 4         // byte

// Variable Declarations
uint8_t USB_RX_BUF[USB_RX_DATA_SIZE];

// ##send_name 是一个预处理器操作符，用于将 send_name 的值与 字符 连接
// 检查是否已经过了一定的时间间隔， 超过时间间隔后，发送消息
#define CheckDurationAndSend(send_name)                                                  \
    do {                                                                                 \
        if ((HAL_GetTick() - LAST_SEND_TIME.##send_name) >= SEND_DURATION_##send_name) { \
            LAST_SEND_TIME.##send_name = HAL_GetTick();                                  \
            UsbSend##send_name##Data();                                                  \
        }                                                                                \
    } while (0)

// 判断USB连接状态用到的一些变量
bool USB_OFFLINE = true;
uint32_t RECEIVE_TIME = 0;
uint32_t LATEST_RX_TIMESTAMP = 0;
uint32_t CONTINUE_RECEIVE_CNT = 0;

// 数据发送结构体

static SendDataAllRobotHp_s     SEND_DATA_ALL_ROBOT_HP;    // 发送全场机器人剩余HP
static SendDataGameStatus_s     SEND_DATA_GAME_STATUS;     // 发送比赛运行状态

static SendDataGameBegin_s      SendDataGameBegin;         // 发送允许发弹量

static SendDataRadarMark_s		SEND_DATA_RADARMARK;	   // 发送雷达标记进度
static SendDataVulnerable_s		SEND_DATA_VULNERABLE;	   // 发送雷达易伤情况  
  
  
  
ReceiveAdvancedData_t ReceiveAdvancedData;


// 发送数据间隔时间
typedef struct
{
    uint32_t AllRobotHp    ;
    uint32_t GameStatus    ;
    uint32_t GameBegin     ;
	uint32_t Radar_Mark	   ;
} LastSendTime_t;
static LastSendTime_t LAST_SEND_TIME;

/*******************************************************************************/
/* Main Function                                                               */
/*******************************************************************************/
static void UsbSendData(void);
static void UsbReceiveData(void);
static void UsbInit(void);
/*******************************************************************************/
/* Send Function                                                               */
/*******************************************************************************/
static void UsbSendAllRobotHpData(void);
static void UsbSendGameStatusData(void);
static void UsbSendGameBeginData(void);
static void UsbSendRadarMarkData(void);
static void UsbSendVulnerableData(void);
/******************************************************************/
/* Task                                                           */
/******************************************************************/
/**
 * @brief      USB任务主函数
 * @param[in]  argument: 任务参数
 * @retval     None
 */
 
TaskHandle_t usb_task_handle;
void usb_task(void *pvParameters)
{
    UsbInit();
    
    portTickType xLastWakeTime;
    xLastWakeTime = xTaskGetTickCount();
    
    for(;;)
    {
        UsbSendData();
        UsbReceiveData();

        if (HAL_GetTick() - RECEIVE_TIME < USB_OFFLINE_THRESHOLD) {
            USB_OFFLINE = true;
            CONTINUE_RECEIVE_CNT = 0;
        } else if (CONTINUE_RECEIVE_CNT > USB_CONNECT_CNT) {
            USB_OFFLINE = false;
        } else {
            CONTINUE_RECEIVE_CNT++;
        }
        
        if(USB_OFFLINE == false)
        {
            memset(&USB_RX_BUF, 0, sizeof(USB_RX_BUF));
        }
        vTaskDelayUntil(&xLastWakeTime,2);
    }
}


/*******************************************************************************/
/* Main Function                                                               */
/*******************************************************************************/
/**
 * @brief      USB初始化
 * @param      None
 * @retval     None
 */
static void UsbInit(void)
{
    // 数据置零
    memset(&LAST_SEND_TIME, 0, sizeof(LastSendTime_t));
	memset(&ReceiveAdvancedData, 0, sizeof(ReceiveAdvancedData_t));

    // 1.初始化全场机器人剩余HP数据包
    SEND_DATA_ALL_ROBOT_HP.frame_header.sof = SEND_SOF;
    SEND_DATA_ALL_ROBOT_HP.frame_header.len = (uint8_t)(sizeof(SendDataAllRobotHp_s) - 6);
    SEND_DATA_ALL_ROBOT_HP.frame_header.id = ALL_ROBOT_HP_SEND_ID;
    append_CRC8_check_sum(  // 添加帧头 CRC8 校验位
        (uint8_t *)(&SEND_DATA_ALL_ROBOT_HP.frame_header), sizeof(SEND_DATA_ALL_ROBOT_HP.frame_header));
    
	// 2.雷达标记进度数据
	SEND_DATA_RADARMARK.frame_header.sof = SEND_SOF;
	SEND_DATA_RADARMARK.frame_header.len = (uint8_t)(sizeof(SendDataRadarMark_s) - 6);
	SEND_DATA_RADARMARK.frame_header.id = RADAR_MARK_SEND_ID;
    append_CRC8_check_sum(  // 添加帧头 CRC8 校验位
        (uint8_t *)(&SEND_DATA_RADARMARK.frame_header), sizeof(SEND_DATA_RADARMARK.frame_header));
	
    // 3.初始化比赛运行状态数据
    SEND_DATA_GAME_STATUS.frame_header.sof = SEND_SOF;
    SEND_DATA_GAME_STATUS.frame_header.len = (uint8_t)(sizeof(SendDataGameStatus_s) - 6);
    SEND_DATA_GAME_STATUS.frame_header.id = GAME_STATUS_SEND_ID;
    append_CRC8_check_sum(  // 添加帧头 CRC8 校验位
        (uint8_t *)(&SEND_DATA_GAME_STATUS.frame_header),sizeof(SEND_DATA_GAME_STATUS.frame_header));

    // 4.游戏状态
    SendDataGameBegin.frame_header.sof = SEND_SOF;
    SendDataGameBegin.frame_header.len = (uint8_t)(sizeof(SendDataGameBegin_s) - 6);
    SendDataGameBegin.frame_header.id = DATA_SEND_ID_GameBegin;
    append_CRC8_check_sum(  // 添加帧头 CRC8 校验位
        (uint8_t *)(&SendDataGameBegin.frame_header),sizeof(SendDataGameBegin.frame_header));
		
		
	SEND_DATA_VULNERABLE.frame_header.sof = SEND_SOF;
	SEND_DATA_VULNERABLE.frame_header.len = (uint8_t)(sizeof(SendDataVulnerable_s) - 6);
	SEND_DATA_VULNERABLE.frame_header.id = Vulnerable_SEND_ID;
    append_CRC8_check_sum(  // 添加帧头 CRC8 校验位
        (uint8_t *)(&SEND_DATA_VULNERABLE.frame_header),sizeof(SEND_DATA_VULNERABLE.frame_header));	
}

/**
 * @brief      用USB发送数据
 * @param      None
 * @retval     None
 */

static void UsbSendData(void)
{
//    UsbSendGameBeginData();//比赛是否开始的标志位 
    UsbSendAllRobotHpData();//全体机器人血量，从这里获得哨兵血量
	UsbSendRadarMarkData();
//	UsbSendVulnerableData();
}

/**
 * @brief      USB接收数据
 * @param      None
 * @retval     None
 */
uint8_t * sof_address;
static void UsbReceiveData(void)
{
    static uint32_t len = USB_RECEIVE_LEN;
    static uint8_t * rx_data_start_address = USB_RX_BUF;  // 接收数据包时存放于缓存区的起始位置
    static uint8_t * rx_data_end_address;  // 接收数据包时存放于缓存区的结束位置
    sof_address = USB_RX_BUF;

    // 计算数据包的结束位置
    rx_data_end_address = rx_data_start_address + USB_RECEIVE_LEN;
    // 读取数据
    USB_Receive(rx_data_start_address, &len);  // Read data into the buffer

    while (sof_address <= rx_data_end_address) {  // 解析缓冲区中的所有数据包
        // 寻找帧头位置
        while (*(sof_address) != RECEIVE_SOF && (sof_address <= rx_data_end_address)) {
            sof_address++;
        }
        // 判断是否超出接收数据范围
        if (sof_address > rx_data_end_address) {
            break;  // 退出循环
        }
        // 检查CRC8校验
        bool crc8_ok = verify_CRC8_check_sum(sof_address, HEADER_SIZE);
        if (crc8_ok) {
            uint8_t data_len = sof_address[1];
            uint8_t data_id = sof_address[2];
            // 检查整包CRC16校验 4: header size, 2: crc16 size
            bool crc16_ok = verify_CRC16_check_sum(sof_address, 4 + data_len + 2);
            if (crc16_ok) {
                switch (data_id) {
					case ReceiveAdvanced_DATA_RECEIVE_ID:{
						memcpy(&ReceiveAdvancedData, sof_address,8);
						uint8_t cnt = data_len/5;
						
						Advanced_Process(&ReceiveAdvancedData, sof_address, cnt);
					}
					break;
                    default:break;
                }
                if (*((uint32_t *)(&sof_address[4])) > LATEST_RX_TIMESTAMP) {
                    LATEST_RX_TIMESTAMP = *((uint32_t *)(&sof_address[4]));
                    RECEIVE_TIME = HAL_GetTick();
                }
            }
            sof_address += (data_len + HEADER_SIZE + 2);
        } else {  //CRC8校验失败，移动到下一个字节
            sof_address++;
        }
    }
    // 更新下一次接收数据的起始位置
    if (sof_address > rx_data_start_address + USB_RECEIVE_LEN) {
        // 缓冲区中没有剩余数据，下次接收数据的起始位置为缓冲区的起始位置
        rx_data_start_address = USB_RX_BUF;
    } else {
        uint16_t remaining_data_len = USB_RECEIVE_LEN - (sof_address - rx_data_start_address);
        // 缓冲区中有剩余数据，下次接收数据的起始位置为缓冲区中剩余数据的起始位置
        rx_data_start_address = USB_RX_BUF + remaining_data_len;
        // 将剩余数据移到缓冲区的起始位置
        memcpy(USB_RX_BUF, sof_address, remaining_data_len);
    }
}

/*******************************************************************************/
/* Send Function                                                               */
/*******************************************************************************/
/**
 * @brief 发送全场机器人hp信息数据
 * @param duration 发送周期
 */
static void UsbSendAllRobotHpData(void)
{
    SEND_DATA_ALL_ROBOT_HP.time_stamp = HAL_GetTick();
    memcpy(&SEND_DATA_ALL_ROBOT_HP.data, &judge_sensor.info->GameRobotHP, sizeof(SEND_DATA_ALL_ROBOT_HP.data));

    append_CRC16_check_sum((uint8_t *)&SEND_DATA_ALL_ROBOT_HP, sizeof(SendDataAllRobotHp_s));
    USB_Transmit((uint8_t *)&SEND_DATA_ALL_ROBOT_HP, sizeof(SendDataAllRobotHp_s));
}

/**
 * @brief 发送雷达标记进度数据
 * @param duration 发送周期
 */
static void UsbSendRadarMarkData(void)
{
	SEND_DATA_RADARMARK.time_stamp = judge_sensor.info->GameStatus.SyncTimeStamp;
	memcpy(&SEND_DATA_RADARMARK.data, &judge_sensor.info->radar_mark_data, sizeof(SEND_DATA_RADARMARK.data));

	append_CRC16_check_sum((uint8_t *)&SEND_DATA_RADARMARK, sizeof(SendDataRadarMark_s));
	USB_Transmit((uint8_t *)&SEND_DATA_RADARMARK, sizeof(SendDataRadarMark_s));
}

/**
 * @brief 发送比赛状态数据
 * @param duration 发送周期
 */
static void UsbSendGameStatusData(void)
{
    SEND_DATA_GAME_STATUS.time_stamp = HAL_GetTick();

    SEND_DATA_GAME_STATUS.data.game_progress = judge_sensor.info->GameStatus.game_progress;
    SEND_DATA_GAME_STATUS.data.stage_remain_time = judge_sensor.info->GameStatus.stage_remain_time;

    append_CRC16_check_sum((uint8_t *)&SEND_DATA_GAME_STATUS, sizeof(SendDataGameStatus_s));
    USB_Transmit((uint8_t *)&SEND_DATA_GAME_STATUS, sizeof(SendDataGameStatus_s));
}

/**
 * @brief 比赛是否开始
 * @param duration 发送周期
 */
static void UsbSendGameBeginData(void)
{
    SendDataGameBegin.time_stamp = HAL_GetTick();

    if(judge_sensor.info->GameStatus.game_progress == 4)
        SendDataGameBegin.IsGameBegin_Flag = 1;
    else
        SendDataGameBegin.IsGameBegin_Flag = 0; 
    
    append_CRC16_check_sum((uint8_t *)&SendDataGameBegin, sizeof(SendDataGameBegin_s));
    USB_Transmit((uint8_t *)&SendDataGameBegin, sizeof(SendDataGameBegin_s));
}

static void UsbSendVulnerableData(void)
{
	SEND_DATA_VULNERABLE.time_stamp = HAL_GetTick();
	
	SEND_DATA_VULNERABLE.data.vulnerable = (judge_sensor.info->radar_info.radar_info & 0x03);
	
    append_CRC16_check_sum((uint8_t *)&SEND_DATA_VULNERABLE, sizeof(SendDataVulnerable_s));
    USB_Transmit((uint8_t *)&SEND_DATA_VULNERABLE, sizeof(SendDataVulnerable_s));	
}




uint8_t hero_visible = 0;
uint8_t engineer_visible = 0;
uint16_t hero = 0;
uint16_t engineer = 0;

/**
 * @brief 上位机数据处理函数
 */
void Advanced_Process(ReceiveAdvancedData_t* ReceiveAdvancedData, uint8_t * sof_address, uint8_t cnt)
{
	hero_visible = 0;
	engineer_visible = 0;
	for(int i=0;i<cnt;i++)
	{
		if(judge_sensor.info->RobotStatus.robot_id == 9)
		{
			switch(sof_address[8 + i * 5]){
				case hero_id:
					hero_visible = 1;
					hero = 0;
					if(judge_sensor.info->GameRobotHP.blue_1_robot_HP != 0)
					{
						ReceiveAdvancedData->hero_position_x 		= (sof_address[8+5*i+1] | (sof_address[8+5*i+2] << 8)) & 0xffff;
						ReceiveAdvancedData->hero_position_y 		= (sof_address[8+5*i+3] | (sof_address[8+5*i+4] << 8)) & 0xffff;
					}
					break;
				case engineer_id:
					engineer_visible = 1;
					engineer = 0;
					if(judge_sensor.info->GameRobotHP.blue_2_robot_HP != 0)
					{
						ReceiveAdvancedData->engineer_position_x 	= (sof_address[8+5*i+1] | (sof_address[8+5*i+2] << 8)) & 0xffff;
						ReceiveAdvancedData->engineer_position_y 	= (sof_address[8+5*i+3] | (sof_address[8+5*i+4] << 8)) & 0xffff;
					}
					break;
				case infantry_3_id:
					if(judge_sensor.info->GameRobotHP.blue_3_robot_HP != 0)
					{
						ReceiveAdvancedData->infantry_3_position_x 	= (sof_address[8+5*i+1] | (sof_address[8+5*i+2] << 8)) & 0xffff;
						ReceiveAdvancedData->infantry_3_position_y 	= (sof_address[8+5*i+3] | (sof_address[8+5*i+4] << 8)) & 0xffff;
					}
					break;
				case infantry_4_id:
					if(judge_sensor.info->GameRobotHP.blue_4_robot_HP != 0)
					{
						ReceiveAdvancedData->infantry_4_position_x 	= (sof_address[8+5*i+1] | (sof_address[8+5*i+2] << 8)) & 0xffff;
						ReceiveAdvancedData->infantry_4_position_y 	= (sof_address[8+5*i+3] | (sof_address[8+5*i+4] << 8)) & 0xffff;
					}
					break;
				case infantry_5_id:
					if(judge_sensor.info->GameRobotHP.blue_5_robot_HP != 0)
					{
						ReceiveAdvancedData->infantry_5_position_x 	= (sof_address[8+5*i+1] | (sof_address[8+5*i+2] << 8)) & 0xffff;
						ReceiveAdvancedData->infantry_5_position_y 	= (sof_address[8+5*i+3] | (sof_address[8+5*i+4] << 8)) & 0xffff;
					}
					break;
				case sentry_id:
					if(judge_sensor.info->GameRobotHP.blue_7_robot_HP != 0)
					{
						ReceiveAdvancedData->sentry_position_x 		= (sof_address[8+5*i+1] | (sof_address[8+5*i+2] << 8)) & 0xffff;
						ReceiveAdvancedData->sentry_position_y 		= (sof_address[8+5*i+3] | (sof_address[8+5*i+4] << 8)) & 0xffff;
					}
					break;
				default:break;
			}
			if(hero_visible == 0)
			{
				hero++;
			}
			if(hero_visible == 0 && hero >= 10)
			{
				if(judge_sensor.info->GameRobotHP.blue_outpost_HP != 0)
				{
					ReceiveAdvancedData->hero_position_x = 1788;
					ReceiveAdvancedData->hero_position_y = 1093;
				}
				else 
				{
					ReceiveAdvancedData->hero_position_x = 1788;
					ReceiveAdvancedData->hero_position_y = 1093;
				}
			}		
			if(engineer_visible == 0)
			{
				engineer++;
			}
			if(engineer_visible == 0 && engineer >= 10)
			{
				ReceiveAdvancedData->engineer_position_x = 2581;
				ReceiveAdvancedData->engineer_position_y = 1404;
			}
		}
		else if(judge_sensor.info->RobotStatus.robot_id == 109)
		{
			switch(sof_address[8 + i * 5]){
				case hero_id:
					hero_visible = 1;
					hero = 0;
					if(judge_sensor.info->GameRobotHP.red_1_robot_HP != 0)
					{
						ReceiveAdvancedData->hero_position_x 		= (sof_address[8+5*i+1] | (sof_address[8+5*i+2] << 8)) & 0xffff;
						ReceiveAdvancedData->hero_position_y 		= (sof_address[8+5*i+3] | (sof_address[8+5*i+4] << 8)) & 0xffff;
					}
					break;
				case engineer_id:
					engineer_visible = 1;
					engineer = 0;
					if(judge_sensor.info->GameRobotHP.red_2_robot_HP != 0)
					{
						ReceiveAdvancedData->engineer_position_x 	= (sof_address[8+5*i+1] | (sof_address[8+5*i+2] << 8)) & 0xffff;
						ReceiveAdvancedData->engineer_position_y 	= (sof_address[8+5*i+3] | (sof_address[8+5*i+4] << 8)) & 0xffff;
					}
					break;
				case infantry_3_id:
					if(judge_sensor.info->GameRobotHP.red_3_robot_HP != 0)
					{
						ReceiveAdvancedData->infantry_3_position_x 	= (sof_address[8+5*i+1] | (sof_address[8+5*i+2] << 8)) & 0xffff;
						ReceiveAdvancedData->infantry_3_position_y 	= (sof_address[8+5*i+3] | (sof_address[8+5*i+4] << 8)) & 0xffff;
					}
					break;
				case infantry_4_id:
					if(judge_sensor.info->GameRobotHP.red_4_robot_HP != 0)
					{
						ReceiveAdvancedData->infantry_4_position_x 	= (sof_address[8+5*i+1] | (sof_address[8+5*i+2] << 8)) & 0xffff;
						ReceiveAdvancedData->infantry_4_position_y 	= (sof_address[8+5*i+3] | (sof_address[8+5*i+4] << 8)) & 0xffff;
					}
					break;
				case infantry_5_id:
					if(judge_sensor.info->GameRobotHP.red_5_robot_HP != 0)
					{
						ReceiveAdvancedData->infantry_5_position_x 	= (sof_address[8+5*i+1] | (sof_address[8+5*i+2] << 8)) & 0xffff;
						ReceiveAdvancedData->infantry_5_position_y 	= (sof_address[8+5*i+3] | (sof_address[8+5*i+4] << 8)) & 0xffff;
					}
					break;
				case sentry_id:
					if(judge_sensor.info->GameRobotHP.red_7_robot_HP != 0)
					{
						ReceiveAdvancedData->sentry_position_x 		= (sof_address[8+5*i+1] | (sof_address[8+5*i+2] << 8)) & 0xffff;
						ReceiveAdvancedData->sentry_position_y 		= (sof_address[8+5*i+3] | (sof_address[8+5*i+4] << 8)) & 0xffff;
					}
					break;
				default:break;
			}
			if(hero_visible == 0)
			{
				hero++;
			}
			if(hero_visible == 0 && hero >= 10)
			{
				if(judge_sensor.info->GameRobotHP.red_outpost_HP != 0)
				{
					ReceiveAdvancedData->hero_position_x = 1027;
					ReceiveAdvancedData->hero_position_y = 1038;
				}
				else 
				{
					ReceiveAdvancedData->hero_position_x = 1006;
					ReceiveAdvancedData->hero_position_y = 419;
				}
			}
			if(engineer_visible == 0)
			{
				engineer++;
			}
			if(engineer_visible == 0 && engineer >= 10)
			{
				ReceiveAdvancedData->engineer_position_x = 210;
				ReceiveAdvancedData->engineer_position_y = 105;
			}
		}
	}
}






