...
>由速度矢量计算电机速度,该函数是弱函数,可以重写以适应不同的底盘（原本是麦轮底盘，我改了全向轮）
...
* 添加了一个flag，可解决开机圈数出现问题的情况
* 增加了达妙电机一拖四模式下的解包，和温度判断函数
...
改：增加了通过步进达到斜坡现象的函数，此函数可不受时间限制，分为整型和浮点型
...
改：CRC校验已修改，将队库crc校验（返回crc校验值）与大疆官方crc校验（返回布尔量）融合
