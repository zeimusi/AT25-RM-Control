...
int float_rounding(float raw)
{
    ...
    integer = (int)raw;      //raw的整数部分
    decimal = raw - integer; //rawd的小数部分
    if (decimal > 0.5f)
        integer++;
    return integer;
}
...
void OLS_Init(Ordinary_Least_Squares_t *OLS, uint16_t order)
{
    OLS->Order = order;                                     //样本数
    OLS->Count = 0;                                         //计数值
    OLS->x = (float *)user_malloc(sizeof(float) * order);   //样本点
    OLS->y = (float *)user_malloc(sizeof(float) * order);   //样本点
    OLS->k = 0;                                             //斜率
    OLS->b = 0;                                             //截距        
    memset((void *)OLS->x, 0, sizeof(float) * order);       //内存清零
    memset((void *)OLS->y, 0, sizeof(float) * order);
    memset((void *)OLS->t, 0, sizeof(float) * 4);//用于存储临时计算结果                               
}
...
void OLS_Update(Ordinary_Least_Squares_t *OLS, float deltax, float y)
{
    static float temp = 0;
    temp = OLS->x[1];
    //将数组中所有数据左移（因为数据是从数组右侧开始计入的），最后一次计入时，数据左移同时减去第一次计入的时间
