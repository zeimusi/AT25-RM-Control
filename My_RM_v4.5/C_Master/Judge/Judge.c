...
typedef enum
{
    ...
    ID_ROBOT_POS			 = 0x0203,	// 机器人位置
    ID_HURT_DATA			 = 0x0206,	// 机器人伤害状态
    ID_SHOOT_DATA			 = 0x0207,	// 实时射击信息
    ID_PROJECTILE_ALLOWANCE	 = 0x0208,	// 子弹剩余发射数
    ID_RFID_STATUS			 = 0x0209,	// 机器人RFID状态
    ID_Robot_Position        = 0x020B,  // 地面机器人位置
    ID_SENTRY_INFO           = 0x020D,  // 哨兵成功兑弹/兑血量
    ID_map_command           = 0x0303,  // 云台手下发数据
} Judge_Cmd_ID_t;
...
typedef enum
{
    ...
    LEN_RFID_STATUS                 = 4,
    LEN_Robot_Position              = 40,
    LEN_SENTRY_INFO                 = 6,
    LEN_map_command                 = 11,
} Judge_Data_Length_t;


extern uint8_t flag_IsMapCommand;
            ...
            if(verify_CRC16_check_sum(rxBuf, frame_length) == true)
            {
                ...
                switch(cmd_id)
                {
                    ...
                    case ID_POWER_HEAT_DATA:
                        ...
                        break;
                    case ID_ROBOT_POS:
                        memcpy(&judge_info->RobotPos, (rxBuf + DATA_SEG), LEN_ROBOT_POS);
                        break;
                    case ID_HURT_DATA:
                        memcpy(&judge_info->HurtData, (rxBuf + DATA_SEG), LEN_HURT_DATA);
                        break;
                    case ID_SHOOT_DATA:
                        memcpy(&judge_info->ShootData, (rxBuf + DATA_SEG), LEN_SHOOT_DATA);
                        break;
                    case ID_PROJECTILE_ALLOWANCE:
                        memcpy(&judge_info->ProjectileAllowance, (rxBuf + DATA_SEG), LEN_PROJECTILE_ALLOWANCE);
                        break;
                    case ID_RFID_STATUS:
                        memcpy(&judge_info->RfidStatus, (rxBuf + DATA_SEG), LEN_RFID_STATUS);
                        break;
                    case ID_Robot_Position:
                        memcpy(&judge_info->RobotPosition, (rxBuf + DATA_SEG), LEN_Robot_Position);
                        break;
                    case ID_SENTRY_INFO:
                        memcpy(&judge_info->SentryInfo, (rxBuf + DATA_SEG), LEN_SENTRY_INFO);
                        break;
                    case ID_map_command:
                        memcpy(&judge_info->map_command, (rxBuf + DATA_SEG),LEN_map_command);
                        flag_IsMapCommand = 1;
                        break;
                    default:break;
                    }
