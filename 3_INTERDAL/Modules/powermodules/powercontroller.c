...
const static float CAP_BASE_BUFFSET     = 40.0f;

/* 裁判系统缓冲能量 */
const float REFEREE_FULL_BUFFSET        = 60.0f;  // 裁判系统最大缓冲能量
const float REFEREE_BASE_BUFFSET        = 40.0f;  // 功率限制使用的基础缓冲能量  作为PID的期望使用缓冲功率
/* 功率重分配阈值(期望速度误差) */
...
static PID powerPD_base = {.Kp = POWER_PD_KP, .Ki = 0.0f, .Kd = POWER_PD_KD, .interlimit = 100, .outlimit = 120, .DeadBand = 0.1f, .inter_threLow = 5, .inter_threUp = 20},
		   ...
		   powerPD_buff = {.Kp = 15,          .Ki = 0.0f, .Kd = 0.8,       .interlimit = 100, .outlimit = 60,  .DeadBand = 0.1f, .inter_threLow = 5, .inter_threUp = 20};
		...
		for (uint8_t i = 0; i < 4 ; i++)
		{
			if (DJIMotor_detect(Chassis_Manager.chassis_motor[i]) == RM_SUCCESS) {
				...
				if (floatEqual(delta, 0.0f))   // 重复根 repeat roots
					...
					newTorqueCurrent[i] = -po->curAv / (2.0f * Chassis_Manager.k2 ) / k0; // imaginary roots  虚解 （或可以直接赋值为0）
...
//		  SuperCap_Rx.cap_power = 55;
#if USE_SUPER_CAPACITOR == 1 // 使用超电
		  if ( !isFlagged(&Chassis_Manager.error, CAPDisConnect) && SuperCap_Rx.cap_power  >= 55.0f)  // 超电在线使用缓冲能量
                   receive_action.Key == 2   ? (Chassis_Manager.powerUpperLimit = Chassis_Manager.refereeMaxPower + 220 )  
                         : ( Chassis_Manager.powerUpperLimit = Chassis_Manager.refereeMaxPower + 20 + MAX_CAP_POWER_OUT );//- PID_Control(sqrtf(chassis_power_get.send_power.power_buffer), sqrtf(REFEREE_BASE_BUFFSET), &powerPD_buff)*0.9f);
          ...
          limit(Chassis_Manager.powerUpperLimit , 340, 0);
#else
...
// Since we have less available feedback, we constrain the power conservatively
		...
		if(isFlagged(&Chassis_Manager.error, CAPDisConnect) && isFlagged(&Chassis_Manager.error, RefereeDisConnect)) {
		   ...
		   PID_IoutReset(&powerPD_full);
            ...
            receive_action.Key != 2    ? (Chassis_Manager.baseMaxPower = fmax(Chassis_Manager.refereeMaxPower - PID_Control(sqrtf(Chassis_Manager.powerBuff), sqrtf(Chassis_Manager.baseBuffSet), &powerPD_base), MIN_MAXPOWER_CONFIGURED) )
                     : ( Chassis_Manager.baseMaxPower = Chassis_Manager.refereeMaxPower + 240);
	   ...
	   /** 向超级电容更新功率 **/
		...
		if(SuperCap_Rx.cap_percent == 0 && SuperCap_Rx.cap_v != 0) {
            ...
            if(isCapEnergyOut == TRUE)
               SuperCAP_Send(&hcan2, 0x210,  (int16_t)(chassis_power_get.send_power.power_limit - 5), referee_get_data.buffer_energy); // 超电正常运行
	    }
