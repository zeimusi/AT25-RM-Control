...
#define IIC_DEVICE_CNT 2 //使用IIC2 和 IIC3
#define MX_IIC_SLAVE_CNT 8 // 最大从机数目
...
typedef struct iic_temp_s{
	...
	uint8_t dev_address;        // 暂时只支持7为地址(最后以为是读写位)
	
	IIC_Work_Mode_e work_mode;  // 工作模式
	uint8_t *rx_buff;           // 接收缓冲区指针
	uint8_t rx_len;             // 接收长度
	void (*callback)(struct iic_temp_s *); // 接收回调函数
