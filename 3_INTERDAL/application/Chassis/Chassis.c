...
//static PID Chassis_Speed_PID[4] = {{.Kp = 20.0f, .Ki = 0.00, .Kd = 0.0f, .interlimit = 3000, .outlimit = 15000, .DeadBand = 0.0f, .inter_threLow = 500, .inter_threUp = 1000},  /* 左前轮 */
//								{.Kp = 20.0f, .Ki = 0.00, .Kd = 0.0f, .interlimit = 3000, .outlimit = 15000, .DeadBand = 0.0f, .inter_threLow = 500, .inter_threUp = 1000},  /* 右前轮 */
//								{.Kp = 20.0f, .Ki = 0.00, .Kd = 0.0f, .interlimit = 3000, .outlimit = 15000, .DeadBand = 0.0f, .inter_threLow = 500, .inter_threUp = 1000},  /* 右后轮 */
//								{.Kp = 20.0f, .Ki = 0.00, .Kd = 0.0f, .interlimit = 3000, .outlimit = 15000, .DeadBand = 0.0f, .inter_threLow = 500, .inter_threUp = 1000}}; /* 左后轮 */

static PID Chassis_Speed_PID[4] = {{.Kp = 12.0f, .Ki = 0.00, .Kd = 0, .interlimit = 3000, .outlimit = 15000, .DeadBand = 0.0f, .inter_threLow = 500, .inter_threUp = 1000},  /* 左前轮 */
								{.Kp = 12.0f, .Ki = 0.00, .Kd = 0, .interlimit = 3000, .outlimit = 15000, .DeadBand = 0.0f, .inter_threLow = 500, .inter_threUp = 1000},  /* 右前轮 */
								{.Kp = 12.0f, .Ki = 0.00, .Kd = 0, .interlimit = 3000, .outlimit = 15000, .DeadBand = 0.0f, .inter_threLow = 500, .inter_threUp = 1000},  /* 右后轮 */
								{.Kp = 12.0f, .Ki = 0.00, .Kd = 0, .interlimit = 3000, .outlimit = 15000, .DeadBand = 0.0f, .inter_threLow = 500, .inter_threUp = 1000}}; /* 左后轮 */
...
{
		// 底盘期望速度斜坡
	Slope_Init(&slope_X, 1.0f / 1000.0f, 10.0f / 1000.0f, SLOPE_FIRST_REAL);
...
Motor_Init_config_s RF_config = {
	...
	/**  功率计算  **/
    Manager_Init(chassis_motor, INFANTRY, RLS_Enable, 0.012972983f, 450.0f, 0.75f); 
...
void Self_Resolution(void)
{
    for(uint8_t i = 0; i < 4 ; i++)  
	   currentAv[i] = chassis_motor[i]->measure.SpeedFilter /* (2.0f * PI / 60.0f)*/;  // 将各个轮组转速RPM转化为角速度
	// 底盘实际速度解算
...
//            Rampspeed[i] = RAMP_int16( speed_target[i] * MOTOR_DECELE_RATIO, Rampspeed[i], 15 +  ABS(17             * (8 * measuredVelocity.Omega) * 6));
        if(receive_action.Key != 2)
        {
//            if(targetVelocity.Omega >= 250 || targetVelocity.Omega <= -250)
//                Rampspeed[i] = RAMP_int16( speed_target[i] * MOTOR_DECELE_RATIO, Rampspeed[i], 55 +  ABS( 7 * measuredVelocity.Omega) * referee_get_data.level_gain);
...
//                Rampspeed[i] = RAMP_int16( speed_target[i] * MOTOR_DECELE_RATIO, Rampspeed[i], 55 +  ABS( 8 * measuredVelocity.Omega) * referee_get_data.level_gain);
//            else if(targetVelocity.Omega < 250 || targetVelocity.Omega > -250)
//                Rampspeed[i] = RAMP_int16( speed_target[i] * MOTOR_DECELE_RATIO, Rampspeed[i], 55 * referee_get_data.level_gain);
 

					if(targetVelocity.Omega >= 250 || targetVelocity.Omega <= -250)
                Rampspeed[i] = RAMP_int16( speed_target[i] * MOTOR_DECELE_RATIO, Rampspeed[i], 30 +  ABS( 7 * measuredVelocity.Omega) * referee_get_data.level_gain);
            else if((targetVelocity.Omega >= 250 || targetVelocity.Omega <= -250) && (chassis_cmd_recv.vx > 1205 || chassis_cmd_recv.vy > 1205 || chassis_cmd_recv.vx < -1205  || chassis_cmd_recv.vy < -1205))
                Rampspeed[i] = RAMP_int16( speed_target[i] * MOTOR_DECELE_RATIO, Rampspeed[i], 30 +  ABS( 8 * measuredVelocity.Omega) * referee_get_data.level_gain);
            else if(targetVelocity.Omega < 250 || targetVelocity.Omega > -250)
                Rampspeed[i] = RAMP_int16( speed_target[i] * MOTOR_DECELE_RATIO, Rampspeed[i], 30 * referee_get_data.level_gain);

						//                Rampspeed[i] = RAMP_int16( speed_target[i] * MOTOR_DECELE_RATIO, Rampspeed[i], 35 * referee_get_data.level_gain);

        }
        else
            Rampspeed[i] = RAMP_int16( speed_target[i] * MOTOR_DECELE_RATIO, Rampspeed[i], 20 * referee_get_data.level_gain); 

//        else if(chassis_cmd_recv.vx >= 1100)
//            Rampspeed[i] = RAMP_int16( speed_target[i] * MOTOR_DECELE_RATIO, Rampspeed[i], 20); 
...
//        Rampspeed[i] = RAMP_int16( speed_target[i] * MOTOR_DECELE_RATIO, Rampspeed[i], 45);
		...
		if(Rampspeed[i] == 0 && fabs(chassis_motor[i]->measure.SpeedFilter) < 350.0f)
...
void Chassis_Upeadt()
{
	...
	if(SubGetMessage(chassis_sub, (void *)&chassis_cmd_recv) != 1 ) {
		...
		if(chassis_cmd_recv.Close_flag == 1 )
			...
			targetVelocity.Omega  =  (referee_get_data.level_gain - 0.25) * chassis_cmd_recv.rotate*1.0f;
