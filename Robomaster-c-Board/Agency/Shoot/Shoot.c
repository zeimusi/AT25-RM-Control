...
PID_Smis Pluck_Place_PIDS = {.Kp = 15, .Ki = 0, .Kd = - 0.8, .limit = 5000};         //拨弹盘单发位置环
PID Pluck_Speed_PID = {.Kp = 20, .Ki = 0, .Kd = 0, .limit = 5000};                   
...
void ShootRef_Set(){
    ...
    case SHOOT_NORMAL:
		if(Add_Angle_Flag == 1){
		 ...
		 SHOOT.Ref_2006_Angle    =  SHOOT.Angle_DEG;
		 Angle_Target            = SHOOT.Angle_DEG + PLUCK_MOTOR_ONE;      //拨弹盘期望角度 = 当前拨弹盘角度 + 一个弹丸的位置
			}
		 Add_Angle_Flag = 0;
			break;
    case SHOOT_RUNNING:
...
void Shoot_Console(){
    ...
    if((SHOOT.Action == SHOOT_NORMAL)){
			if(SHOOT.Action == SHOOT_NORMAL)SHOOT.Ref_2006_Angle = RAMP_float (Angle_Target, SHOOT.Ref_2006_Angle, 15);
...
void Detect_Shoot(){
    ...
    if(SHOOT.Action != SHOOT_STOP){
     ...
     if(Stuck_time >= 50){                    // 卡弹时间超过阈值便认为卡弹需退弹
		 ...
		 if(ABS( Pluck_Motor.Speed ) > 200 && Stuck_time > 100){
         Stuck_time = 0;
         SHOOT.Action = SHOOT_RUNNING;
		 }
	}
...
@brief 枪口热量限制
*/
        float Recover,Remain,Consumption,shoot_speed;//冷却速率，热量剩余，单发消耗的热量，射速 发/s
	      uint16_t ShootTime,shoot_time,K = 1;
				uint8_t reduction_ratio = 36,caliper = 8;//减速比，拨尺
...
//					if(Referee_data_Rx.game_state == 1){
								if(shoot_time == 0){
										...
										//分级射速
//										if(Remain < 100){
//											shoot_speed = (10 * Remain - Recover - 3 * Consumption) / (Consumption * ShootTime) + Recover / Consumption;
//										}else{
											shoot_speed = ((10 * Remain - Recover - Consumption) / (Consumption*(ShootTime/100.0)) + Recover / Consumption);
//										}
										
								}
								else if(0 < shoot_time && shoot_time < ShootTime){
											SHOOT.Ref_2006 = shoot_speed * reduction_ratio * 60 / caliper;
								}else{
											SHOOT.Ref_2006 = Recover / Consumption * reduction_ratio * 60 / caliper;
