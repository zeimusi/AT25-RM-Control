...
void GimbalInit(){
     ...
     PID_init(&Gimbal_Place_pid_Yaw[MECH],1000,1,0,2.0,0,0,0,0,0,0,Integral_Limit );	
	   PID_init(&Gimbal_Speed_pid_Yaw[MECH],25000,400,0,200,0,400,0,0,0,0,Integral_Limit);
//     PID_init(&Gimbal_Place_pid_Yaw[GYRO],25000,0.1,0,0.3,0.005,0,0,0,0,0,Integral_Limit);	
//	   PID_init(&Gimbal_Speed_pid_Yaw[GYRO],25000,0,0,6000,0,3000,0,0,0,0,Integral_Limit);	
     PID_init(&Gimbal_Place_pid_Yaw[GYRO],25000,0.1,0,0.2,0.0015,0,0,0,0,0,Integral_Limit);	
	   PID_init(&Gimbal_Speed_pid_Yaw[GYRO],25000,0,0,12000,0,8000,0,0,0,0,Integral_Limit);	
     PID_init(&Gimbal_Place_pid_Yaw[AIM],25000,0.2,0,0.25,0,0,0,0,0,0,Integral_Limit);	
	   PID_init(&Gimbal_Speed_pid_Yaw[AIM],25000,5000,0,14000,0,5000,0,0,0,0,Integral_Limit);	
////     PID_init(&Gimbal_Place_pid_Yaw[AIM],25000,1000,0,0,0,0,0,0,0,0,Integral_Limit);	
////	   PID_init(&Gimbal_Speed_pid_Yaw[AIM],25000,0,0,-1,0,0,0,0,0,0,Integral_Limit);	

	   PID_init(&Gimbal_Speed_pid_Pitch[MECH],25000,0,0,4,0,0,0,0,0,0,Integral_Limit);	
	   PID_init(&Gimbal_Place_pid_Pitch[MECH],50000,100,0,0.5,0,0,0,0,0,0,Integral_Limit);
	   PID_init(&Gimbal_Speed_pid_Pitch[GYRO],25000,1,0,3,0,0,0,0,0,0,Integral_Limit);	
	   PID_init(&Gimbal_Place_pid_Pitch[GYRO],50000,1,0,0.5,0,0,0,0,0,0,Integral_Limit);
...
switch(GimbalCtrl){		
	...
	case gNormal:
		...
		if(RemoteMode == REMOTE_INPUT){
			...
			Gimbal.increase[PITCH]  = Mouse_ch[1] * 0.15;
		  }
		}
...
//		Gimbal.LastCtrl = gNormal;
		limit(Gimbal.Ref[Mech].Pitch,Gimbal.PitchInit + P_ADD_limit,Gimbal.PitchInit - P_LOSE_limit);		 
	break;
	...
	case gAim:
	  ...
	  if(Gimbal.Mode == Gyro && ReceiveVisionData.data.tracking){
...
void Gimbal_Pid(){
	   float FeedForwardYawSpeed;//小陀螺时的yaw轴抗扰前馈
		 ...
		 if(GimbalCtrl == gAim)Gimbal_Place_pid_Yaw[AIM].Output = Gimbal_Place_pid_Yaw[AIM].Output ;//+ FeedForwardYawSpeed * 0.135;
		 ...
		 /* PITCH轴 */
		 if(GimbalCtrl == gAim && ReceiveVisionData.data.tracking)
		{
...
void Gimbal_Send(){
	   ...
	   if(RC_CtrlData.rc.s2 == 2){
		 ...
		 if(GimbalCtrl == gAim && ReceiveVisionData.data.tracking == 1)
			 Can2Send_DM4310[PITCH] = (int16_t)(Gimbal_Speed_pid_Pitch[GYRO].Output);
     else Can2Send_DM4310[PITCH] =(int16_t)(Gimbal_Speed_pid_Pitch[MECH].Output);
#if GIMBAL_RUN
		 if(RemoteMode != STOP){
				MotorSend(&hcan2, 0x1FF, Can2Send_Gimbal);
	      MotorSend(&hcan2, 0x3fe, Can2Send_DM4310);
		 }
#endif
...
void MedianInit(){
     ...
     if (Time.GimbalInit < 10){
        ...
        if ( (Gimbal_Motor[YAW].MchanicalAngle <= Yaw_Mid_Left) || (Gimbal_Motor[YAW].MchanicalAngle >= Yaw_Mid_Right) )
         ...
         /* Yaw轴归中 */       
         Expect_YawRamp = RAMP_float(Expect_YawInit,Expect_YawRamp,80); 
         ...
         Expect_PitchRamp = RAMP_float(Pitch_Mid,Expect_PitchRamp,15); 
         ...
         Can2SendDM4310[PITCH] = (int16_t)Gimbal_Speed_pid_Pitch[INIT].Output;
			 

 #if GIMBAL_RUN
         MotorSend(&hcan2, 0x1FF, Can2Send);
	       MotorSend(&hcan2, 0x3fe, Can2SendDM4310);			 
 #endif
     }		 
...
//					 Gimbal_Motor[PITCH].r = 0;
           ...
           Ramp_PitchRefG                   = Gimbal_Motor[PITCH].Angle_DEG;
					 ...
					 Gimbal.PitchInit                 = Gimbal_Motor[PITCH].Angle_DEG;
...
//										Feed_Dog(&Referee_Dog);
        ...
        case 0x205: GM6020_Receive( &Gimbal_Motor[YAW], CAN2_buff); 
                    Feed_Dog(&Gimbal_Dog[YAW]);
                    break;
			
        case 0x302: DM4310_Receive(&DM4310_Pitch, CAN2_buff); 
										DMmotor_to_DJmotor();
...
void DMmotor_to_DJmotor(){
	   Gimbal_Motor[PITCH].Angle = DM4310_Pitch.Angle;
	   Gimbal_Motor[PITCH].Angle_DEG = DM4310_Pitch.Angle_DEG;
	   Gimbal_Motor[PITCH].LsatAngle = DM4310_Pitch.LsatAngle;
		 Gimbal_Motor[PITCH].MchanicalAngle = DM4310_Pitch.MchanicalAngle;
	   Gimbal_Motor[PITCH].r = DM4310_Pitch.r;
	   Gimbal_Motor[PITCH].Speed = DM4310_Pitch.Speed;
		 Gimbal_Motor[PITCH].temp = DM4310_Pitch.temp;
		 Gimbal_Motor[PITCH].TorqueCurrent = DM4310_Pitch.TorqueCurrent;
}
